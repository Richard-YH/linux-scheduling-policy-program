...
int main(int argc, char **argv) {
    ...
    unsigned int priority_min; 
    unsigned int priority_max;
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        param.sched_priority = thread_info[i].sched_policy;
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        if ( pthread_setaffinity_np(thread_info[i].thread_id, sizeof(cpu_set_t), &cpuset) ) {
            perror("pthread_setaffinity_np");
            printf("Error setting CPU affinity for thread %ld\n", thread_info[i].thread_id);
        }
    }
